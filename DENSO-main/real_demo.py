...
MODEL_PATH = r"D:\Personal\DENSO_Project\project\DENSO\best_1.pt"
CONF_THRES    = 0.40
CONF_TRIGGER  = 0.65
STABLE_FRAMES = 7
...
def main():
    ...
    while True:
        if locked_coords is not None:
            Xr, Zr = locked_coords
            print("\n" + "∞"*30)
            print(f"🚀 THỰC HIỆN CHUỖI: X={Xr:.1f} Z={Zr:.1f}")
            print("∞"*30)
            
            # --- CHUỖI HÀNH ĐỘNG THEO YÊU CẦU ---
            
            # Bước 2-3-4: Đến vật -> Hạ -> Kẹp -> Nhấc
            robot.approach_and_pick(Xr, Zr, z_table=Zr + BOX_DEPTH/2)
            
            # Bước 5-6: Quay phải -> Đặt xuống -> Mở tay
            robot.place_to_right()
            
            # Bước 7: Về vị trí init (Home)
        ...
        if H is not None:
            ...
            for r in results:
                for box in r.boxes:
                    ...
                    cv2.putText(frame, f"X={Xr:.1f} Z={Zr:.1f}", (x1, y2+20),
                    ...
                    if conf >= CONF_TRIGGER:
                        ...
                        if stable_count >= STABLE_FRAMES:
                            locked_coords = (Xr, Zr)
                            # 2. Nhận diện tọa độ vật phẩm (đã xong, chuyển sang loop xử lý)
                            print(f"\n✅ LOCKED: X={Xr:.1f} Z={Zr:.1f}")
